...
const TimeLinePage = () => {
    ...
    const [filterInfo, setFilterInfo] = useState(quickCheck)
    const handleFilter = (info) =>{

        // console.log(info, 'filter data')
        const expectedFilterInfo = quickCheck.filter(data => data.type == info)

        info === 'all' ? setFilterInfo(quickCheck) : setFilterInfo(expectedFilterInfo)
        // console.log(filterInfo, expectedFilterInfo, info)
    ...
    return (
        ...
        quickCheck.length === 0 ? 
            ...
            (<div className='container mx-auto space-y-6 px-4 py-10  md:p-16  lg:p-20'>
                <h2 className='text-3xl  lg:text-5xl font-bold '>Timeline {filterInfo.length}</h2>
                ...
                <select defaultValue="Filter Timeline All" className="select border-zinc-200 outline-hidden">
                    ...
                    <option onClick={() => handleFilter('text')}>Filter: Text</option>
                    <option onClick={() => handleFilter('call')}>Filter: Call</option>
                ...
                <div className='space-y-6'>
                    {
                        ...
                        filterInfo.map((friend, ind) => <TimeLineStatus key={ind} friend={friend}></TimeLineStatus>)
                    }
                </div>
            </div>)
